...
static inline int
sndfileFormatInfoFromStrings(struct SF_INFO *info, const char *headerFormatString, const char *sampleFormatString)
{
...
makeSndfileHandle(const char *path, int mode = SFM_READ, int format = 0, int channels = 0, int samplerate = 0)
{
    const std::wstring path_w = SC_Codecvt::utf8_cstr_to_utf16_wstring(path);
