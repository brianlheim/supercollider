...
void Trig1_next(Trig1* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float curtrig = ZXP(trig); float zout; if (counter > 0) { zout = --counter ? 1.f : 0.f; } else {
...
void Trig1_next_k(Trig1* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float zout; if (counter > 0) { zout = --counter ? 1.f : 0.f; } else {
        if (curtrig > 0.f && prevtrig <= 0.f) {
            counter = (long)(dur * sr + .5f);
            if (counter < 1)
                counter = 1;
...
void Trig1_next_nova(Trig1* unit, int inNumSamples)
{
    ...
    if (counter > inNumSamples) {
        ...
        LOOP1(inNumSamples, float curtrig = ZXP(trig); float zout;
...
void Trig_next_k(Trig* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float zout; if (counter > 0) { zout = --counter ? level : 0.f; } else {
        if (curtrig > 0.f && prevtrig <= 0.f) {
            counter = (long)(dur * sr + .5f);
            if (counter < 1)
                counter = 1;
...
void SendTrig_next(SendTrig* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float curtrig = ZXP(trig);
          if (curtrig > 0.f && prevtrig <= 0.f) { SendTrigger(&unit->mParent->mNode, (int)ZIN0(1), ZIN0(2)); } prevtrig
          = curtrig;);
    unit->m_prevtrig = prevtrig;
}
...
void Latch_next_aa(Latch* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float curtrig = ZXP(trig); if (prevtrig <= 0.f && curtrig > 0.f) level = ZXP(in);
...
void Gate_next_aa(Gate* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float curtrig = ZXP(trig); if (curtrig > 0.f) level = ZXP(in);
...
void PulseCount_next_k(PulseCount* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float curtrig = ZXP(trig); if (prevreset <= 0.f && curreset > 0.f) level = 0.f;
          else if (prevtrig <= 0.f && curtrig > 0.f) { level += 1.f; } ZXP(out) = level; prevtrig = curtrig;
          prevreset = curreset;);
    unit->mLevel = level;
    unit->m_prevtrig = prevtrig;
...
void Stepper_next_aa(Stepper* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float curtrig = ZXP(trig); float curreset = ZXP(reset);
          if (prevreset <= 0.f
              && curreset > 0.f) { level = (float)sc_wrap(resetval, zmin, zmax); } else if (prevtrig <= 0.f
              && curtrig > 0.f) { level = (float)sc_wrap((int32)level + step, zmin, zmax); } ZXP(out)
          = level;
...
void Stepper_next_a0(Stepper* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float curtrig = ZXP(trig);
          if (prevtrig <= 0.f && curtrig > 0.f) { level = (float)sc_wrap((int32)level + step, zmin, zmax); } ZXP(out)
          = level;
          prevtrig = curtrig;);
    unit->mLevel = level;
    unit->m_prevtrig = prevtrig;
...
void PeakFollower_next(PeakFollower* unit, int inNumSamples)
{
    ...
    if (decay == unit->mDecay) {
        LOOP1(inNumSamples, float inlevel = std::abs(ZXP(in));
              if (inlevel >= level) { level = inlevel; } else { level = inlevel + decay * (level - inlevel); } ZXP(out)
              = level;);
        ...
        if (decay >= 0.f && unit->mDecay >= 0.f) {
            LOOP1(inNumSamples, float inlevel = std::abs(ZXP(in)); if (inlevel >= level) { level = inlevel; } else {
...
void MostChange_next_ak(MostChange* unit, int inNumSamples)
{
    ...
    LOOP1(
        inNumSamples, float xa = ZXP(a); float diff = std::abs(xa - prevA) - std::abs(xb - prevB); if (diff > 0.f) {
            ...
            recent = 1;
            ZXP(out) = xb;
        } else { ZXP(out) = recent ? xb : xa; } prevA
        = xa;
        prevB = xb;);
    unit->mPrevA = prevA;
    unit->mPrevB = prevB;
