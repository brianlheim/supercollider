...
enum { diskinIdle, diskinStartingEmpty, diskinStartingFull, diskinNormal, diskinLastBuffer, diskinEndSilence };

enum { diskoutIdle, diskoutNormal };
...
struct DiskIOThread
{
    ...
    void launchThread()
    {
        ...
        mThread = thread(bind(&DiskIOThread::ioThreadFunc, this));
    }
...
void DiskIn_next(DiskIn *unit, int inNumSamples)
{
    ...
    if (unit->m_buf->mask1 >= 0 && unit->m_framepos >= unit->m_buf->mask1)
        unit->mDone = true;
    ...
    sendMessage:
        if (unit->m_buf->mask >= 0)
            unit->m_buf->mask1 = unit->m_buf->mask;
        if (unit->mWorld->mRealTime) {
            ...
            if (mPos > (uint32)bufr->frames || mPos + bufFrames2 > (uint32)bufr->frames ||
                (uint32)bufr->channels != bufChannels)
                return;
            sf_count_t count;
            ...
            if ((int)ZIN0(1)) { // loop
                ...
                if (count < bufFrames2) {
                    memset(
                        bufr->data + (mPos + count) * bufr->channels, 0,
...
static void VDiskIn_request_buffer(VDiskIn *unit, float fbufnum, uint32 bufFrames2, uint32 bufChannels, double bufPos)
{
    if (unit->m_buf->mask >= 0)
        unit->m_buf->mask1 = unit->m_buf->mask;
    unit->m_count++;
...
template <bool First> static inline void VDiskIn_next_(VDiskIn *unit, int inNumSamples)
{
    bool test = false;
    ...
    for (int j = firstLoopSample; j < inNumSamples; ++j) {
        ...
        while (table1 >= bufSamples)
            table1 -= bufSamples;
        while (table0 < 0)
            table0 += bufSamples;
        while (table2 >= bufSamples)
            table2 -= bufSamples;
        while (table3 >= bufSamples)
            table3 -= bufSamples;
        for (uint32 i = 0; i < bufChannels; i++) {
            float a, b, c, d;
            a = bufData[table0 + i];
    ...
    if (unit->m_buf->mask1 >= 0 && bufPos >= unit->m_buf->mask1)
        unit->mDone = true;
    if (test)
...
void VDiskIn_first(VDiskIn *unit, int inNumSamples) { VDiskIn_next_<true>(unit, inNumSamples); }

void VDiskIn_next(VDiskIn *unit, int inNumSamples) { VDiskIn_next_<false>(unit, inNumSamples); }
