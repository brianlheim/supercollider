...
void LFPulse_next_a(LFPulse* unit, int inNumSamples)
{
    ...
    LOOP1(
        inNumSamples, float z; if (phase >= 1.f) {
            ...
            z = duty <= 0.5f ? 1.f : 0.f;
        } else { z = phase < duty ? 1.f : 0.f; } phase
        += ZXP(freq) * freqmul;
        ZXP(out) = z;);
...
void VarSaw_next_a(VarSaw* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples,
          ...
          = phase < duty ? phase * invduty : (1.f - phase) * inv1duty;
...
static inline void LinExp_next_nova_loop(float* out, const float* in, int inNumSamples, nova::vec<float> dstlo,
    nova::vec<float> dstratio, nova::vec<float> rsrcrange, nova::vec<float> rrminuslo)
{
...
static inline bool check_gate_ar(
    EnvGen* unit, int i, float& prevGate, float*& gatein, int& nsmps, int& counter, double level)
{
