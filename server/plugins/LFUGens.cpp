...
void Impulse_next_ik(Impulse *unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, ZXP(out) = Impulse_testWrapPhase(phaseInc, phase);
...
struct DC : SIMD_Unit
{
    ...
    DC(void)
    {
        value = in0(0);
        if (value == 0)
            set_unrolled_calc_function<DC,
                                       &DC::next_i<unrolled_64, true>,
                                       &DC::next_i<unrolled, true>,
                                       &DC::next_i<scalar, true>>();
        else
            set_unrolled_calc_function<DC,
                                       &DC::next_i<unrolled_64, false>,
                                       &DC::next_i<unrolled, false>,
