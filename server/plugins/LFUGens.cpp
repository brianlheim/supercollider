...
void LFCub_next_a(LFCub *unit, int inNumSamples)
{
    ...
    float freqmul = unit->mFreqMul;
    double phase = unit->mPhase;
    LOOP1(inNumSamples, float z; if (phase < 1.f) { z = phase; } else if (phase < 2.f) { z = 2.f - phase; } else {
        phase -= 2.f;
        z = phase;
    } float phaseadd = ZXP(freq);
...
void SyncSaw_next_aa(SyncSaw *unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float freq1x = ZXP(freq1) * freqmul; float freq2x = ZXP(freq2) * freqmul; float z = phase2;
...
void SyncSaw_next_ak(SyncSaw *unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float freq1x = ZXP(freq1) * freqmul; float z = phase2; phase2 += freq2x;
...
void SyncSaw_next_ka(SyncSaw *unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float freq2x = ZXP(freq2) * freqmul; float z = phase2; phase2 += freq2x;
          if (phase2 >= 1.f) phase2 -= 2.f; phase1 += freq1x; if (phase1 >= 1.f) {
              phase1 -= 2.f;
              phase2 = (phase1 + 1.f) * freq2x / freq1x - 1.f;
          } ZXP(out) = z;);
...
void SyncSaw_next_kk(SyncSaw *unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float z = phase2; phase2 += freq2x; if (phase2 >= 1.f) phase2 -= 2.f; phase1 += freq1x;
          if (phase1 >= 1.f) {
              phase1 -= 2.f;
              phase2 = (phase1 + 1.f) * freq2x / freq1x - 1.f;
          } ZXP(out) = z;);
    ...
    DC(void)
    {
        ...
        if (value == 0)
            set_unrolled_calc_function<
                DC, &DC::next_i<unrolled_64, true>, &DC::next_i<unrolled, true>, &DC::next_i<scalar, true>>();
        else
            set_unrolled_calc_function<
                DC, &DC::next_i<unrolled_64, false>, &DC::next_i<unrolled, false>, &DC::next_i<scalar, false>>();
    }
...
#ifdef NOVA_SIMD
static inline void LinExp_next_nova_loop(
    float *out, const float *in, int inNumSamples, nova::vec<float> dstlo, nova::vec<float> dstratio,
...
static inline bool
check_gate_ar(EnvGen *unit, int i, float &prevGate, float *&gatein, int &nsmps, int &counter, double level)
{
