...
void WrapBufRd_next0(Unit *unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float fpos = ZXP(pos); int ipos = (int)fpos * numchan; ipos = sc_mod(ipos, numframes);
...
void ClipBufRd_next0(Unit *unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float fpos = ZXP(pos); int ipos = (int)fpos * numchan; ipos = sc_clip(ipos, 0, maxframe);
          int index = numchan * ipos; for (int i = 0; i < numchan; ++i) {
              *++(out[i]) = data[index];
              index++;
          });
