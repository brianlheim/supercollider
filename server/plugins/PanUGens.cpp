...
void LinPan2_next_ak(LinPan2* unit, int inNumSamples)
{
    ...
    if (pos != unit->m_pos || unit->m_level != level) {
        ...
        LOOP1(inNumSamples, float zin = ZXP(in); ZXP(leftout) = zin * leftamp; ZXP(rightout) = zin * rightamp;
              leftamp += leftampslope; rightamp += rightampslope;);
        unit->m_pos = pos;
        unit->m_level = level;
...
void LinPan2_next_ak_nova(LinPan2* unit, int inNumSamples)
{
    ...
    if (pos != unit->m_pos || unit->m_level != level) {
        ...
        nova::pan2_vec_simd(OUT(0), OUT(1), IN(0), leftamp, leftampslope, rightamp, rightampslope, inNumSamples);
        unit->m_pos = pos;
        unit->m_level = level;
        unit->m_leftamp = nextleftamp;
        unit->m_rightamp = nextrightamp;
...
void LinPan2_next_ak_nova_64(LinPan2* unit, int inNumSamples)
{
    ...
    if (pos != unit->m_pos || unit->m_level != level) {
        ...
        nova::pan2_vec_simd<64>(OUT(0), OUT(1), IN(0), leftamp, leftampslope, rightamp, rightampslope);
        unit->m_pos = pos;
        unit->m_level = level;
        unit->m_leftamp = nextleftamp;
        unit->m_rightamp = nextrightamp;
...
void LinPan2_next_aa(LinPan2* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float pan = ZXP(pos) * 0.5f + 0.5f; float rightamp = level * pan;
          float leftamp = level - rightamp; float zin = ZXP(in); ZXP(leftout) = zin * leftamp;
...
void Balance2_next_ak(Balance2* unit, int inNumSamples)
{
    ...
    if (pos != unit->m_pos || unit->m_level != level) {
        ...
        LOOP1(inNumSamples, ZXP(leftout) = ZXP(leftin) * leftamp; ZXP(rightout) = ZXP(rightin) * rightamp;
              leftamp += leftampslope; rightamp += rightampslope;);
        unit->m_pos = pos;
        unit->m_level = level;
        unit->m_leftamp = nextleftamp;
...
void Balance2_next_ak_nova(Balance2* unit, int inNumSamples)
{
    ...
    if (pos != unit->m_pos || unit->m_level != level) {
        ...
        // nova::times_vec2_ramp_simd(OUT(0), IN(0), leftamp, leftampslope, OUT(1), IN(1), rightamp, rightampslope,
        // inNumSamples);
...
void Balance2_next_aa(Balance2* unit, int inNumSamples)
{
    ...
    if (level != nextlevel) {
        float levelSlope = (nextlevel - level) * unit->mRate->mSlopeFactor;
        LOOP1(inNumSamples, int32 ipos = (int32)(1024.f * ZXP(pos) + 1024.f + 0.5f); ipos = sc_clip(ipos, 0, 2048);

              float leftamp = level * sineTable[2048 - ipos]; float rightamp = level * sineTable[ipos];
              ZXP(leftout) = ZXP(leftin) * leftamp; ZXP(rightout) = ZXP(rightin) * rightamp; level += levelSlope;);
        unit->m_level = level;
    } else {
        LOOP1(inNumSamples, int32 ipos = (int32)(1024.f * ZXP(pos) + 1024.f + 0.5f); ipos = sc_clip(ipos, 0, 2048);

              float leftamp = level * sineTable[2048 - ipos]; float rightamp = level * sineTable[ipos];
              ZXP(leftout) = ZXP(leftin) * leftamp; ZXP(rightout) = ZXP(rightin) * rightamp;);
    }
}
...
void XFade2_next_ak(XFade2* unit, int inNumSamples)
{
    ...
    if (pos != unit->m_pos || unit->m_level != level) {
        ...
        LOOP1(inNumSamples, ZXP(out) = ZXP(leftin) * leftamp + ZXP(rightin) * rightamp;);
    }
}
...
void XFade2_next_ak_nova(XFade2* unit, int inNumSamples)
{
    ...
    if (pos != unit->m_pos || unit->m_level != level) {
        ...
        nova::mix_vec_simd(OUT(0), IN(0), slope_argument(leftamp, leftampslope), IN(1),
            slope_argument(rightamp, rightampslope), inNumSamples);
...
void XFade2_next_ak_nova_64(XFade2* unit, int inNumSamples)
{
    ...
    if (pos != unit->m_pos || unit->m_level != level) {
        ...
        nova::mix_vec_simd<64>(
            OUT(0), IN(0), slope_argument(leftamp, leftampslope), IN(1), slope_argument(rightamp, rightampslope));
...
void XFade2_next_aa(XFade2* unit, int inNumSamples)
{
    ...
    if (level != nextlevel) {
        ...
        LOOP1(inNumSamples, int32 ipos = (int32)(1024.f * ZXP(pos) + 1024.f + 0.5f); ipos = sc_clip(ipos, 0, 2048);
              ...
              float leftamp = level * sineTable[2048 - ipos]; float rightamp = level * sineTable[ipos];
              ZXP(out) = ZXP(leftin) * leftamp + ZXP(rightin) * rightamp; level += levelSlope;);
        unit->m_level = level;
    } else {
        LOOP1(inNumSamples, int32 ipos = (int32)(1024.f * ZXP(pos) + 1024.f + 0.5f); ipos = sc_clip(ipos, 0, 2048);
...
void LinXFade2_next_i(LinXFade2* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float l = ZXP(leftin); float r = ZXP(rightin); ZXP(out) = l + amp * (r - l););
}
...
void LinXFade2_next_a(LinXFade2* unit, int inNumSamples)
{
    ...
    LOOP1(inNumSamples, float pos = ZXP(posp); pos = sc_clip(pos, -1.f, 1.f); float amp = pos * 0.5f + 0.5f;
          float l = ZXP(leftin); float r = ZXP(rightin); ZXP(out) = l + amp * (r - l););
}
...
FLATTEN void LinXFade2_next_k_nova(LinXFade2* unit, int inNumSamples)
{
    ...
    if (pos != unit->m_pos) {
        ...
        nova::mix_vec_simd(OUT(0), IN(0), nova::slope_argument(oldAmpLeft, leftSlope), IN(1),
            nova::slope_argument(oldAmpRight, rightSlope), inNumSamples);
    } else
        nova::mix_vec_simd(OUT(0), IN(0), 1.f - amp, IN(1), amp, inNumSamples);
}
...
void Pan2_next_aa(Pan2* unit, int inNumSamples)
{
    ...
    if (level != nextlevel) {
        ...
        LOOP1(inNumSamples, int32 ipos = (int32)(1024.f * ZXP(pos) + 1024.f + 0.5f); ipos = sc_clip(ipos, 0, 2048);
              ...
              float leftamp = level * ft->mSine[2048 - ipos]; float rightamp = level * ft->mSine[ipos];
              float zin = ZXP(in); ZXP(leftout) = zin * leftamp; ZXP(rightout) = zin * rightamp; level += levelSlope;);
        unit->m_level = level;
    } else {
        LOOP1(inNumSamples, int32 ipos = (int32)(1024.f * ZXP(pos) + 1024.f + 0.5f); ipos = sc_clip(ipos, 0, 2048);

              float leftamp = level * ft->mSine[2048 - ipos]; float rightamp = level * ft->mSine[ipos];
              float zin = ZXP(in); ZXP(leftout) = zin * leftamp; ZXP(rightout) = zin * rightamp;);
    }
}
...
void Pan4_next(Pan4* unit, int inNumSamples)
{
    ...
    if (xpos != unit->m_xpos || ypos != unit->m_ypos || level != unit->m_level) {
        ...
        LOOP1(inNumSamples, float z = ZXP(in); ZXP(LFout) = z * LF_amp; ZXP(RFout) = z * RF_amp;
              ZXP(LBout) = z * LB_amp; ZXP(RBout) = z * RB_amp; LF_amp += LF_slope; RF_amp += RF_slope;
              LB_amp += LB_slope; RB_amp += RB_slope;);
        ...
        LOOP1(inNumSamples, float z = ZXP(in); ZXP(LFout) = z * LF_amp; ZXP(RFout) = z * RF_amp;
              ZXP(LBout) = z * LB_amp; ZXP(RBout) = z * RB_amp;);
    }
}
...
void PanB_next(PanB* unit, int inNumSamples)
{
    ...
    if (azimuth != unit->m_azimuth || elevation != unit->m_elevation || level != unit->m_level) {
        ...
        LOOP1(inNumSamples, float z = ZXP(in); ZXP(Wout) = z * W_amp; ZXP(Xout) = z * X_amp; ZXP(Yout) = z * Y_amp;
              ZXP(Zout) = z * Z_amp; W_amp += W_slope; X_amp += X_slope; Y_amp += Y_slope; Z_amp += Z_slope;);
        unit->m_W_amp = W_amp;
        unit->m_X_amp = X_amp;
        unit->m_Y_amp = Y_amp;
        unit->m_Z_amp = Z_amp;
...
void PanB2_next(PanB2* unit, int inNumSamples)
{
    ...
    if (azimuth != unit->m_azimuth || level != unit->m_level) {
        ...
        LOOP1(inNumSamples, float z = ZXP(in); ZXP(Wout) = z * W_amp; ZXP(Xout) = z * X_amp; ZXP(Yout) = z * Y_amp;
              W_amp += W_slope; X_amp += X_slope; Y_amp += Y_slope;);
        unit->m_W_amp = W_amp;
        unit->m_X_amp = X_amp;
        unit->m_Y_amp = Y_amp;
    } else {
...
void BiPanB2_next(BiPanB2* unit, int inNumSamples)
{
    ...
    if (azimuth != unit->m_azimuth || level != unit->m_level) {
        ...
        if (W_slope == 0.f) {
            LOOP1(inNumSamples, float a = ZXP(inA); float b = ZXP(inB); float abdiff = a - b;
                  ZXP(Wout) = (a + b) * W_amp; ZXP(Xout) = abdiff * X_amp; ZXP(Yout) = abdiff * Y_amp; X_amp += X_slope;
                  Y_amp += Y_slope;);
        } else {
            LOOP1(inNumSamples, float a = ZXP(inA); float b = ZXP(inB); float abdiff = a - b;
                  ZXP(Wout) = (a + b) * W_amp; ZXP(Xout) = abdiff * X_amp; ZXP(Yout) = abdiff * Y_amp; W_amp += W_slope;
                  X_amp += X_slope; Y_amp += Y_slope;);
            unit->m_W_amp = W_amp;
        }
        unit->m_X_amp = X_amp;
        unit->m_Y_amp = Y_amp;
    } else {
        LOOP1(inNumSamples, float a = ZXP(inA); float b = ZXP(inB); float abdiff = a - b; ZXP(Wout) = (a + b) * W_amp;
              ZXP(Xout) = abdiff * X_amp; ZXP(Yout) = abdiff * Y_amp;);
    }
}
    ...
    for (int i = 0; i < numOutputs; ++i) {
        ...
        if (nextchanamp == chanamp) {
            if (nextchanamp == 0.f) {
                ...
                float* in = zin0;
                LOOP1(inNumSamples, ZXP(out) = ZXP(in) * chanamp;)
            }
        } else {
            float chanampslope = CALCSLOPE(nextchanamp, chanamp);
            float* in = zin0;
            LOOP1(inNumSamples, ZXP(out) = ZXP(in) * chanamp; chanamp += chanampslope;)
...
void PanAz_next_aa(PanAz* unit, int inNumSamples)
{
    ...
    for (int i = 0; i < numOutputs; ++i) {
        ...
        LOOP1(inNumSamples, float chanpos = ((ZXP(thePos) * alignedPosFac + alignedPosConst) - i) * rwidth;
              chanpos = chanpos - range * sc_floor(rrange * chanpos);
...
void Rotate2_next_ak(Rotate2* unit, int inNumSamples)
{
    ...
    if (pos != unit->m_pos) {
        ...
        LOOP1(inNumSamples, float x = ZXP(xin); float y = ZXP(yin); ZXP(xout) = cost * x + sint * y;
              ZXP(yout) = cost * y - sint * x; sint += sinslope; cost += cosslope;);
        unit->m_pos = pos;
    } else {
        LOOP1(inNumSamples, float x = ZXP(xin); float y = ZXP(yin); ZXP(xout) = cost * x + sint * y;
...
void DecodeB2_next(DecodeB2* unit, int inNumSamples)
{
    ...
    for (int i = 0; i < numOutputs; ++i) {
        ...
        float* Xin = Xin0;
        float* Yin = Yin0;
        LOOP1(inNumSamples, ZXP(out) = ZXP(Win) * W_amp + ZXP(Xin) * X_amp + ZXP(Yin) * Y_amp;);
