...
void BeatTrack2_Ctor(BeatTrack2 *unit)
{
    ...
    unit->m_fullwindowsize = unit->m_temporalwindowsize + 1.0 +
                             0.1; // plus one to cover all phases of the 60bpm based period, and a further 0.1 for
                                  // indexing safety; ie looking at areas around the point you're interested in
...
void finaldecision(BeatTrack2 *unit)
{
    ...
    float bestphase = fmod(
        ((unit->bestphase[bestcandidate] * unit->m_phaseaccuracy) + (unit->m_krlength * (unit->m_amortlength))) /
            (unit->m_period),
        (float)1.0);
    ...
    // time elapsed since a known beat is phase of winner in seconds, to calculation start point, plus time for
    // calculation (120 control blocks) divided by period, modulo 1.0
    unit->m_predictphase = fmod(
        ((unit->bestphase[bestcandidate] * unit->m_phaseaccuracy) + (unit->m_krlength * (unit->m_amortlength)) +
         unit->m_calculationperiod) /
            (unit->m_period),
        (float)1.0);
