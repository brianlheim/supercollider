...
inline int sc_fpclassify(float x) { return std::fpclassify(x); }
...
void CheckBadValues_next(CheckBadValues* unit, int inNumSamples)
{
    ...
    case 1: // post a line on every bad value
        LOOP1(inNumSamples, samp = ZXP(in); classification = sc_fpclassify(samp); switch (classification) {
            case FP_INFINITE:
    ...
    case 2:
        LOOP1(
            inNumSamples, samp = ZXP(in); classification = CheckBadValues_fold_fpclasses(sc_fpclassify(samp));
            if (classification != unit->prevclass) {
                if (unit->sameCount == 0) {
                    ...
                    Print("CheckBadValues: %s found in Synth %d, ID %d (previous %d values were %s)\n",
                          ...
                          (int)id,
                          (int)unit->sameCount,
                          CheckBadValues_fpclassString(unit->prevclass));
                };
                unit->sameCount = 0;
            };
...
#else
inline int CheckBadValues_fold_fpclasses(int fpclass) { return fpclass; }
#endif
