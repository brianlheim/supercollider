...
static inline bool checkBuffer(
    Unit* unit, const float* bufData, uint32 bufChannels, uint32 expectedChannels, int inNumSamples)
{
    if (!bufData)
        goto handle_failure;
...
template <bool Checked = false> struct DelayN_helper {
    ...
    static inline void perform(
        const float*& in, float*& out, float* bufData, long& iwrphase, long idsamp, float frac, long mask)
    {
...
template <bool Checked = false> struct CombN_helper {
    ...
    static inline void perform(
        const float*& in, float*& out, float* bufData, long& iwrphase, long idsamp, long mask, float feedbk)
    {
        long irdphase = iwrphase - idsamp;
