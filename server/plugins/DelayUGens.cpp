...
void MaxLocalBufs_Ctor(MaxLocalBufs *unit)
{
    ...
    if (!parent->localMaxBufNum) {
        ...
        printf(
            "warning: MaxLocalBufs - maximum number of local buffers is already declared (%i) and must remain "
            "unchanged.\n",
...
static inline bool
checkBuffer(Unit *unit, const float *bufData, uint32 bufChannels, uint32 expectedChannels, int inNumSamples)
{
    if (!bufData)
        goto handle_failure;
    ...
    if (expectedChannels > bufChannels) {
        if (unit->mWorld->mVerbosity > -1 && !unit->mDone)
            Print(
                "Buffer UGen channel mismatch: expected %i, yet buffer has %i channels\n", expectedChannels,
                bufChannels);
        goto handle_failure;
    }
    return true;
...
#define SETUP_IN(offset) \
    ...
    if (numInputs != bufChannels) { \
        if (unit->mWorld->mVerbosity > -1 && !unit->mDone) { \
            Print( \
                "buffer-writing UGen channel mismatch: numInputs %i, yet buffer has %i channels\n", numInputs, \
                bufChannels); \
        } \
        unit->mDone = true; \
        ClearUnitOutputs(unit, inNumSamples); \
...
template <bool Checked = false> struct DelayN_helper
{
    ...
    static inline void
    perform(const float *&in, float *&out, float *bufData, long &iwrphase, long idsamp, float frac, long mask)
    {
...
static inline void DelayN_delay_loop(
    float *out, const float *in, long &iwrphase, float dsamp, long mask, float *dlybuf, int inNumSamples, int idelaylen)
{
...
template <bool Checked = false> struct CombN_helper
{
    ...
    static inline void
    perform(const float *&in, float *&out, float *bufData, long &iwrphase, long idsamp, long mask, float feedbk)
    {
        long irdphase = iwrphase - idsamp;
    ...
    static inline void perform(
        const float *&in, float *&out, float *bufData, long &iwrphase, long idsamp, float frac, long mask, float feedbk)
    {
...
void ScopeOut2_Ctor(ScopeOut2 *unit)
{
    ...
    if (!ok) {
        if (unit->mWorld->mVerbosity > -1 && !unit->mDone)
            Print(
                "ScopeOut2: Requested scope buffer unavailable! (index: %d, channels: %d, size: %d)\n", scopeNum,
