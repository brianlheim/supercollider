...
void PartConv_Ctor(PartConv* unit)
{
    ...
    unit->m_scfft = scfft_create(
        unit->m_fftsize, unit->m_fftsize, kRectWindow, unit->m_inputbuf, unit->m_spectrum, kForward, alloc);
    ...
    // in place this time
    unit->m_scifft = scfft_create(
        unit->m_fftsize, unit->m_fftsize, kRectWindow, unit->m_inputbuf2, unit->m_spectrum2, kBackward, alloc);
    ...
    if (unit->m_nover2 % unit->m_blocksize != 0) {
        ...
        unit->m_lastamort = (unit->m_partitions - 1)
            - ((unit->m_spareblocks - 1) * (unit->m_numamort)); // allow for error on last one
