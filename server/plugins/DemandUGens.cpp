...
enum { duty_dur, duty_reset, duty_doneAction, duty_level };
...
void Dbrown_next(Dbrown* unit, int inNumSamples)
{
    if (inNumSamples) {
        ...
        float step = DEMANDINPUT_A(3, inNumSamples);
        if (!sc_isnan(step)) {
            unit->m_step = step;
        }
...
void Dshuf_Dtor(Dshuf* unit) { RTFree(unit->mWorld, unit->m_indices); }
...
void Dpoll_next(Dpoll* unit, int inNumSamples)
{
    if (inNumSamples) {
        ...
        if (IN0(1) >= 0.0)
            SendTrigger(&unit->mParent->mNode, (int)IN0(1), x);
        OUT0(0) = x;
    } else {
        RESETINPUT(0);
    }
...
void Dpoll_Dtor(Dpoll* unit) { RTFree(unit->mWorld, unit->m_id_string); }
...
void Dreset_next(Dreset* unit, int inNumSamples)
{
    if (inNumSamples) {
        ...
        if (reset > 0.0 && (unit->prev_reset <= 0.0)) {
            RESETINPUT(0);
        }
