...
bool define_unit(
    const char *inUnitClassName, size_t inAllocSize, UnitCtorFunc inCtor, UnitDtorFunc inDtor, uint32 inFlags)
{
    try {
...
void read_channel(
    SndfileHandle &sf, uint32_t channel_count, const uint32_t *channel_data, uint32 total_frames, sample *data)
{
...
void sc_plugin_interface::buffer_alloc_read_channels(
    uint32_t index, const char *filename, uint32_t start, uint32_t frames, uint32_t channel_count,
...
int sc_plugin_interface::buffer_write(
    uint32_t index, const char *filename, const char *header_format, const char *sample_format, uint32_t start,
    uint32_t frames, bool leave_open)
{
    SndBuf *buf = World_GetNRTBuf(&world, index);
...
void sc_plugin_interface::buffer_read(
    uint32_t index, const char *filename, uint32_t start_file, uint32_t frames, uint32_t start_buffer, bool leave_open)
{
    SndBuf *buf = World_GetNRTBuf(&world, index);
...
void sc_plugin_interface::buffer_read_channel(
    uint32_t index, const char *filename, uint32_t start_file, uint32_t frames, uint32_t start_buffer, bool leave_open,
