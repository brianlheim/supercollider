...
void send_done_message(endpoint_ptr const& endpoint)
{
    ...
    p << osc::BeginMessage("/done") << osc::EndMessage;
...
void fire_node_reply(int32_t node_id, int reply_id, movable_string&& cmd, movable_array<float>&& values)
{
...
void sc_osc_handler::start_receive_udp()
{
    ...
    sc_notify_observers::udp_socket.async_receive_from(buffer(recv_buffer_), udp_remote_endpoint_,
        bind(&sc_osc_handler::handle_receive_udp, this, asio::placeholders::error,
...
sc_osc_handler::received_packet* sc_osc_handler::received_packet::alloc_packet(
    const char* data, size_t length, endpoint_ptr const& remote_endpoint)
{
...
inline void verify_argument(
    osc::ReceivedMessageArgumentIterator const& it, osc::ReceivedMessageArgumentIterator const& end)
{
    if (it == end)
...
template <bool realtime> void handle_b_allocReadChannel(ReceivedMessage const& msg, endpoint_ptr endpoint)
{
    ...
    cmd_dispatcher<realtime>::fire_system_callback(
        [=, message = std::move(message), channel_mapping = std::move(channel_mapping),
...
template <bool realtime> void handle_d_load(ReceivedMessage const& msg, endpoint_ptr endpoint)
{
    ...
    cmd_dispatcher<realtime>::fire_system_callback(
        [=, message = std::move(message), path_string = std::move(path_string)]() mutable {
            ...
            cmd_dispatcher<realtime>::fire_rt_callback(
                [=, message = std::move(message), path_string = std::move(path_string),
                    wrappedSynthdefs = std::move(wrappedSynthdefs)]() mutable {
                    for (sc_synth_definition_ptr& definition : wrappedSynthdefs)
                        instance->register_definition(std::move(definition));
...
void handle_asynchronous_command(World* world, const char* cmdName, void* cmdData, AsyncStageFn stage2,
    AsyncStageFn stage3, AsyncStageFn stage4, AsyncFreeFn cleanup, completion_message&& message, endpoint_ptr endpoint)
{
    cmd_dispatcher<realtime>::fire_system_callback(
...
void sc_osc_handler::do_asynchronous_command(World* world, void* replyAddr, const char* cmdName, void* cmdData,
    AsyncStageFn stage2, AsyncStageFn stage3, AsyncStageFn stage4, AsyncFreeFn cleanup, int completionMsgSize,
