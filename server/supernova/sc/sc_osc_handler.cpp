...
void sc_notify_observers::send_node_reply(
    int32_t node_id, int reply_id, const char *command_name, int argument_count, const float *values)
{
...
void sc_scheduled_bundles::insert_bundle(
    time_tag const &timeout, const char *data, size_t length, endpoint_ptr const &endpoint)
{
...
void sc_osc_handler::start_receive_udp()
{
    ...
    sc_notify_observers::udp_socket.async_receive_from(
        ...
        bind(
            &sc_osc_handler::handle_receive_udp, this, asio::placeholders::error,
...
void sc_osc_handler::tcp_connection::async_read_msg_size()
{
    ...
    asio::async_read(
        socket_, asio::buffer(&msg_size_, 4),
...
void sc_osc_handler::tcp_connection::handle_message_size()
{
    ...
    pointer ptr = std::static_pointer_cast<sc_osc_handler::tcp_connection>(shared_from_this());
    asio::async_read(
        socket_, asio::buffer(msg_buffer_), [=](const boost::system::error_code &error, std::size_t bytes_transferred) {
            if (error == boost::asio::error::eof)
                return; // connection closed
...
sc_osc_handler::received_packet::alloc_packet(const char *data, size_t length, endpoint_ptr const &remote_endpoint)
{
...
template <bool realtime> void sc_osc_handler::handle_bundle(ReceivedBundle const &bundle, endpoint_ptr const &endpoint)
{
...
template <bool realtime> void handle_notify(ReceivedMessage const &message, endpoint_ptr const &endpoint)
{
    ...
    cmd_dispatcher<realtime>::fire_system_callback([=, endpoint = endpoint_ptr(endpoint)]() {
        ...
        if (enable) {
            ...
            if (observer < 0)
                send_fail_message(
                    endpoint, "/notify", sc_notify_observers::error_string((sc_notify_observers::error_code)observer));
        } else {
...
inline void
verify_argument(osc::ReceivedMessageArgumentIterator const &it, osc::ReceivedMessageArgumentIterator const &end)
{
    if (it == end)
...
template <bool realtime> void handle_b_allocReadChannel(ReceivedMessage const &msg, endpoint_ptr endpoint)
{
    ...
    cmd_dispatcher<realtime>::fire_system_callback([=, message = std::move(message),
        ...
        try {
            sc_factory->buffer_alloc_read_channels(
                bufnum, filename.c_str(), start, frames, channel_mapping.size(), channel_mapping.data());
            ...
            cmd_dispatcher<realtime>::free_in_rt_thread(
                std::move(message), std::move(channel_mapping), std::move(filename));
...
fire_callback:
    ...
    cmd_dispatcher<realtime>::fire_system_callback([=, message = std::move(message),
                                                    filenameString = std::move(filenameString),
                                                    headerString = std::move(headerString),
                                                    sampleString = std::move(sampleString)]() mutable {
        sc_ugen_factory::buffer_lock_t buffer_lock(sc_factory->buffer_guard(bufnum));
        try {
            sc_factory->buffer_write(
                bufnum, filenameString.c_str(), headerString.c_str(), sampleString.c_str(), start, frames, leave_open);
        ...
        cmd_dispatcher<realtime>::free_in_rt_thread(
            std::move(message), std::move(filenameString), std::move(headerString), std::move(sampleString));
    });
}
...
fire_callback:
    ...
    cmd_dispatcher<realtime>::fire_system_callback([=, message = std::move(message), filename = std::move(fname),
        ...
        try {
            sc_factory->buffer_read_channel(
                bufnum, filename.c_str(), start_file, frames, start_buffer, leave_open, channel_map.size(),
                channel_map.data());
            ...
            cmd_dispatcher<realtime>::free_in_rt_thread(
                std::move(filename), std::move(channel_map), std::move(message));
        }
    });
}
...
template <bool realtime>
void sc_osc_handler::handle_message_int_address(
    ReceivedMessage const &message, size_t msg_size, endpoint_ptr const &endpoint)
{
...
void dispatch_buffer_commands(
    const char *address, ReceivedMessage const &message, size_t msg_size, endpoint_ptr const &endpoint)
{
    assert(address[1] == 'b');
    assert(address[2] == '_');
...
template <bool realtime>
void sc_osc_handler::handle_message_sym_address(
    ReceivedMessage const &message, size_t msg_size, endpoint_ptr const &endpoint)
{
    const char *address = message.AddressPattern();
...
template <bool realtime>
void handle_asynchronous_command(
    World *world, const char *cmdName, void *cmdData, AsyncStageFn stage2, AsyncStageFn stage3, AsyncStageFn stage4,
    AsyncFreeFn cleanup, completion_message &&message, endpoint_ptr endpoint)
{
    cmd_dispatcher<realtime>::fire_system_callback(
...
void sc_osc_handler::do_asynchronous_command(
    World *world, void *replyAddr, const char *cmdName, void *cmdData, AsyncStageFn stage2, AsyncStageFn stage3,
    AsyncStageFn stage4, AsyncFreeFn cleanup, int completionMsgSize, void *completionMsgData)
{
    ...
    if (world->mRealTime)
        handle_asynchronous_command<true>(
            world, cmdName, cmdData, stage2, stage3, stage4, cleanup, std::move(msg), shared_endpoint);
    else
        handle_asynchronous_command<false>(
            world, cmdName, cmdData, stage2, stage3, stage4, cleanup, std::move(msg), shared_endpoint);
}
