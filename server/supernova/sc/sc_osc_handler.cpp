...
template <typename DerivedClass> struct cmd_dispatcher_base
{
    ...
    {
        DerivedClass::fire_rt_callback([object1 = std::move(object1),
                                        object2 = std::move(object2),
                                        object3 = std::move(object3),
                                        object4 = std::move(object4)]() mutable {
            consume(std::move(object1));
            consume(std::move(object2));
            consume(std::move(object3));
...
void sc_osc_handler::start_receive_udp()
{
    ...
    sc_notify_observers::udp_socket.async_receive_from(buffer(recv_buffer_),
                                                       udp_remote_endpoint_,
                                                       bind(&sc_osc_handler::handle_receive_udp,
                                                            this,
...
fire_callback:
    ...
    cmd_dispatcher<realtime>::fire_system_callback(
        [=, message = std::move(message), filename = std::move(fname), channel_map = std::move(channel_map)]() mutable {
            try {
                sc_factory->buffer_read_channel(bufnum,
                                                filename.c_str(),
                                                start_file,
                                                frames,
                                                start_buffer,
