...
public:
    ...
    void add_node(server_node* node, node_position_constraint const& constraint, Functor const& doOnFree)
    {
        ...
        case replace: {
            ...
            if (reference_node_is_group) {
                ...
                reference_group->apply_deep_on_children([&](server_node& node) { doOnFree(node); });
            }
    ...
    template <typename Functor> void remove_node(server_node* node, Functor const& doOnFree)
    {
        if (!node->is_synth())
...
public:
    ...
    template <typename Functor> void group_free_all(abstract_group* group, Functor const& doOnFree)
    {
    ...
    template <typename Functor> void group_free_deep(abstract_group* group, Functor const& doOnFree)
    {
        size_t synths = 0;
        group->apply_deep_on_children([&](server_node& node) {
            if (node.is_synth()) {
...
private:
    struct equal_node {
        bool operator()(server_node const& lhs, int32_t const& rhs) const { return lhs.node_id == rhs; }

        bool operator()(int32_t const& lhs, server_node const& rhs) const { return lhs == rhs.node_id; }
    };

    struct hash_node {
        std::size_t operator()(int32_t value) { return server_node::hash(value); }
    };

    typedef boost::intrusive::unordered_set<server_node, boost::intrusive::constant_time_size<false>,
