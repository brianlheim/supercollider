    ...
    template <node_position Position> static void move_to_head_or_tail(server_node* node, abstract_group* target)
    {
        ...
        if (node_parent != target) {
            if (node->is_synth()) {
                ...
                target->child_groups.splice(
                    group_target_iterator, node_parent->child_groups, group_list::s_iterator_to(*node_as_group));
            }
            node->parent_ = target;
        }
    ...
    template <node_position Relation> static void move_before_or_after(server_node* node, server_node* target)
    {
        ...
        if (node_parent != target_parent) {
            if (node->is_synth()) {
                ...
                target_parent->child_groups.splice(
                    target_parent->child_groups.end(), node_parent->child_groups, group_target_iterator);
            }
