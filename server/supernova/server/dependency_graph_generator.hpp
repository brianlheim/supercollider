...
private:
    ...
    get_previous_activation_count(reverse_iterator it, reverse_iterator end, int previous_activation_limit)
    {
    ...
    successor_container
    fill_queue_recursive(abstract_group &grp, successor_container const &successors, size_t activation_limit)
    {
        if (grp.is_parallel())
    ...
    HOT successor_container
    fill_queue_recursive(group &g, successor_container const &successors_from_parent, size_t previous_activation_limit)
    {
        ...
        parallel_group &g, successor_container const &successors_from_parent, size_t previous_activation_limit)
    {
        ...
        for (auto &node: g.child_nodes) {
            if (node.is_synth()) {
                thread_queue_item *q_item = q->allocate_queue_item(
                    queue_node(queue_node_data(static_cast<abstract_synth *>(&node))), successors_from_parent,
