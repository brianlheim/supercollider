...
server_arguments::server_arguments(int argc, char* argv[])
{
    ...
    options.add_options()("help,h", "show this help")("udp-port,u", value<uint32_t>(&udp_port)->default_value(0),
        "udp port")("tcp-port,t", value<uint32_t>(&tcp_port)->default_value(0), "tdp port")(
        ...
        "block-size,z", value<uint32_t>(&blocksize)->default_value(64), "audio block size")("hardware-buffer-size,Z",
        value<int32_t>(&hardware_buffer_size)->default_value(0), "hardware buffer size")("use-system-clock,C",
        value<uint16_t>(&use_system_clock)->default_value(0), "type of clock sampleclock=0 systemclock=1")(
        ...
        "max-logins,l", value<uint32_t>()->default_value(64), "maximum number of named return addresses")("password,p",
        value<string>(&server_password)->default_value(""),
        "When using TCP, the session password must be the first command sent.\n"
        "The default is no password.\n"
        "UDP ports never require passwords, so for security use TCP.")("nrt,N", value<vector<string>>()->multitoken(),
        ...
        "print the supercollider version and exit")("hardware-device-name,H", value<vector<string>>()->multitoken(),
