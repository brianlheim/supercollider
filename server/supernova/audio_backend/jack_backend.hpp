...
public:
    void open_client(
        std::string const &server_name, std::string const &name, uint32_t input_port_count, uint32_t output_port_count,
        uint32_t blocksize)
    {
        ...
        client = server_name.empty() ? jack_client_open(name.c_str(), JackNoStartServer, &status)
                                     : jack_client_open(
                                           name.c_str(), jack_options_t(JackNoStartServer | JackServerName), &status,
                                           server_name.c_str());
        ...
        for (uint32_t i = 0; i != input_port_count; ++i) {
            ...
            if (!jack_uuid_empty(uuid)) {
                ...
                jack_set_property(
                    client, uuid, JACKEY_ORDER, order.c_str(), "http://www.w3.org/2001/XMLSchema#integer");
            }
#endif
        }
