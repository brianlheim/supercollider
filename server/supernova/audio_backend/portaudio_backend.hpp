...
public:
    ...
    bool open_stream(
        std::string const &input_device, unsigned int inchans, std::string const &output_device, unsigned int outchans,
        unsigned int samplerate, unsigned int pa_blocksize, int h_blocksize)
    {
        ...
        PaError opened = Pa_OpenStream(
            &stream, in_stream_parameters, out_stream_parameters, samplerate, pa_blocksize, paNoFlag,
...
private:
    ...
    int perform(
        const void *inputBuffer, void *outputBuffer, unsigned long frames, const PaStreamCallbackTimeInfo *timeInfo,
    ...
    static int pa_process(
        const void *input, void *output, unsigned long frame_count, const PaStreamCallbackTimeInfo *time_info,
