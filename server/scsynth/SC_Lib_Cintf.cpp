...
static bool PlugIn_Load(const bfs::path &filename)
{
    ...
    if (!hinstance) {
        ...
        FormatMessage(
            FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, lastErr,
            MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (char *)&s, 0, NULL);
