...
template <typename T, typename... Args> static bool protectedOpenPort(const char* context, Args&&... args) noexcept {
    try {
        ...
        scprintf("\n*** Unknown error while %s\n", context);
    }
    return false;
}

SCSYNTH_DLLEXPORT_C int World_OpenUDP(struct World* inWorld, const char* bindTo, int inPort) {
    return protectedOpenPort<SC_UdpInPort>("opening UDP port (World_OpenUDP)", inWorld, bindTo, inPort);
}

SCSYNTH_DLLEXPORT_C int World_OpenTCP(struct World* inWorld, const char* bindTo, int inPort, int inMaxConnections,
                                      int inBacklog) {
    return protectedOpenPort<SC_TcpInPort>("opening TCP port (World_OpenTCP)", inWorld, bindTo, inPort,
                                           inMaxConnections, inBacklog);
}
