...
class SC_UdpInPort
{
    ...
    void startReceiveUDP()
    {
        ...
        udpSocket.async_receive_from(
            ...
            boost::bind(
                &SC_UdpInPort::handleReceivedUDP, this, asio::placeholders::error,
...
SCSYNTH_DLLEXPORT_C bool
World_SendPacketWithContext(World *inWorld, int inSize, char *inData, ReplyFunc inFunc, void *inContext)
{
    if (inSize > 0) {
        if (inWorld->mDumpOSC)
...
SCSYNTH_DLLEXPORT_C int
World_OpenTCP(struct World *inWorld, const char *bindTo, int inPort, int inMaxConnections, int inBacklog)
{
    try {
