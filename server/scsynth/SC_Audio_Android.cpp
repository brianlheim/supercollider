...
#ifndef NDEBUG
    scprintf(
        "SC_AndroidJNIAudioDriver::DriverSetup: allocating %i bytes for %i frames\n", audioDataSize,
        mPreferredHardwareBufferFrameSize);
#endif

    if (mWorld->mVerbosity >= 0) {
        scprintf(
            "<-SC_AndroidJNIAudioDriver::Setup world %p, mPreferredHardwareBufferFrameSize %i, mPreferredSampleRate "
            "%i, outNumSamplesPerCallback %i, outSampleRate %g\n",
...
void SC_AndroidJNIAudioDriver::genaudio(short *arri, int numSamplesPassed)
{
    ...
    if ((numFramesPerCallback * numOutputs) != numSamplesPassed)
        scprintf(
            "(numFramesPerCallback * numOutputs) != numSamplesPassed, %i %i\n", numFramesPerCallback, numOutputs,
            numSamplesPassed);
#endif
