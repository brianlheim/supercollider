...
SC_SequencedCommand::SC_SequencedCommand(World *inWorld, ReplyAddress *inReplyAddress):
    mNextStage(1), mWorld(inWorld), mMsgSize(0), mMsgData(0)
{
    if (inReplyAddress)
...
BufGenCmd::BufGenCmd(World *inWorld, ReplyAddress *inReplyAddress):
    SC_SequencedCommand(inWorld, inReplyAddress), mData(0)
{}
...
bool BufAllocReadCmd::Stage2()
{
    ...
    if (!sf) {
        ...
        SendFailureWithIntValue(
            &mReplyAddress, "/b_allocRead", str, mBufIndex); // SendFailure(&mReplyAddress, "/b_allocRead", str);
        scprintf(str);
        return false;
    }
    if (mFileOffset < 0)
...
bool BufReadCmd::Stage2()
{
    ...
    if (!sf) {
        ...
        sprintf(str, "File '%s' could not be opened: %s\n", mFilename, sf_strerror(NULL));
        SendFailureWithIntValue(
            &mReplyAddress, "/b_read", str, mBufIndex); // SendFailure(&mReplyAddress, "/b_read", str);
        scprintf(str);
        return false;
    }
    if (fileinfo.channels != buf->channels) {
        ...
        sf_close(sf);
        sprintf(
            str, "Channel mismatch. File '%s' has %d channels. Buffer has %d channels.\n", mFilename, fileinfo.channels,
            buf->channels);
        SendFailureWithIntValue(
            &mReplyAddress, "/b_read", str, mBufIndex); // SendFailure(&mReplyAddress, "/b_read", str);
        scprintf(str);
...
BufAllocReadChannelCmd::BufAllocReadChannelCmd(World *inWorld, ReplyAddress *inReplyAddress):
    SC_BufReadCommand(inWorld, inReplyAddress), mFreeData(0), mFilename(0)
{}
...
bool BufAllocReadChannelCmd::Stage2()
{
    ...
    if (!sf) {
        ...
        sprintf(str, "File '%s' could not be opened: %s\n", mFilename, sf_strerror(NULL));
        SendFailureWithIntValue(
            &mReplyAddress, "/b_allocReadChannel", str, mBufIndex); // SendFailure(&mReplyAddress, "/b_allocRead", str);
        scprintf(str);
        return false;
    }
    if (mFileOffset < 0)
    ...
    if (mNumChannels == 0) {
        ...
        if (!CheckChannels(fileinfo.channels)) {
            ...
            SendFailureWithIntValue(
                &mReplyAddress, "/b_allocReadChannel", str,
...
BufReadChannelCmd::BufReadChannelCmd(World *inWorld, ReplyAddress *inReplyAddress):
    SC_BufReadCommand(inWorld, inReplyAddress), mFilename(0)
{}
...
bool BufReadChannelCmd::Stage2()
{
    ...
    if (!sf) {
        ...
        sprintf(str, "File '%s' could not be opened: %s\n", mFilename, sf_strerror(NULL));
        SendFailureWithIntValue(
            &mReplyAddress, "/b_readChannel", str, mBufIndex); // SendFailure(&mReplyAddress, "/b_read", str);
        scprintf(str);
        return false;
    }
    ...
    if (mNumChannels > 0) {
        ...
        if (!(CheckChannels(fileinfo.channels))) { // nescivi:  && CheckChannels(buf->channels) (should not check here
            ...
            const char *str = "Channel index out of range.\n";
            SendFailureWithIntValue(
                &mReplyAddress, "/b_readChannel", str, mBufIndex); // SendFailure(&mReplyAddress, "/b_allocRead", str);
            scprintf(str);
            sf_close(sf);
            return false;
        }
...
bool BufWriteCmd::Stage2()
{
    ...
    if (!sf) {
        ...
        sprintf(str, "File '%s' could not be opened: %s\n", mFilename, sf_strerror(NULL));
        SendFailureWithIntValue(
            &mReplyAddress, "/b_write", str, mBufIndex); // SendFailure(&mReplyAddress, "/b_write", str);
        scprintf(str);
        return false;
    }
...
AudioQuitCmd::AudioQuitCmd(World *inWorld, ReplyAddress *inReplyAddress): SC_SequencedCommand(inWorld, inReplyAddress)
{}
    ...
    if (mOnOff) {
        for (auto addr: *hw->mUsers) {
            if (mReplyAddress == addr) {
                ...
                SendFailureWithIntValue(
                    &mReplyAddress, "/notify", "notify: already registered\n", hw->mClientIDdict->at(mReplyAddress));
...
SendReplyCmd::SendReplyCmd(World *inWorld, ReplyAddress *inReplyAddress): SC_SequencedCommand(inWorld, inReplyAddress)
{}
    ...
    AsyncFreeFn cleanup, int completionMsgSize, void *completionMsgData)
{
    ...
    AsyncPlugInCmd *cmd = new (space) AsyncPlugInCmd(
        inWorld, (ReplyAddress *)replyAddr, cmdName, cmdData, stage2, stage3, stage4, cleanup, completionMsgSize,
        completionMsgData);
    if (!cmd)
...
AsyncPlugInCmd::AsyncPlugInCmd(
    ...
    SC_SequencedCommand(inWorld, inReplyAddress),
    mCmdName(cmdName), mCmdData(cmdData), mStage2(stage2), mStage3(stage3), mStage4(stage4), mCleanup(cleanup)
{
