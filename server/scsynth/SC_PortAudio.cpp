...
public:
    ...
    int PortAudioCallback(
        const void *input, void *output, unsigned long frameCount, const PaStreamCallbackTimeInfo *timeInfo,
...
SC_PortAudioDriver::SC_PortAudioDriver(struct World *inWorld):
    SC_AudioDriver(inWorld), mStream(0)
#ifdef SC_PA_USE_DLL
    ,
    mMaxOutputLatency(0.)
#endif
...
static int SC_PortAudioStreamCallback(
    const void *input, void *output, unsigned long frameCount, const PaStreamCallbackTimeInfo *timeInfo,
...
int SC_PortAudioDriver::PortAudioCallback(
    const void *input, void *output, unsigned long frameCount, const PaStreamCallbackTimeInfo *timeInfo,
    PaStreamCallbackFlags statusFlags)
{
    sc_SetDenormalFlags();
    ...
    if (++tick >= 10) {
        tick = 0;
        scprintf(
            "DLL: t %.6f p %.9f sr %.6f e %.9f avg(e) %.9f inc %.9f\n", mDLL.PeriodTime(), mDLL.Period(),
...
bool SC_PortAudioDriver::DriverSetup(int *outNumSamples, double *outSampleRate)
{
    ...
    for (int i = 0; i < numDevices; i++) {
        ...
        apiInfo = Pa_GetHostApiInfo(pdi->hostApi);
        fprintf(
            stdout, "  - %s : %s   (device #%d with %d ins %d outs)\n", apiInfo->name, pdi->name, i,
    ...
    *outNumSamples = mWorld->mBufLength;
    ...
    if (mDeviceInOut[0] != paNoDevice || mDeviceInOut[1] != paNoDevice) {
        ...
        paerror = Pa_OpenStream(
            &mStream, inStreamParams_p, outStreamParams_p, *outSampleRate, *outNumSamples, paNoFlag,
    ...
    paerror = Pa_OpenDefaultStream(
        &mStream, mWorld->mNumInputs, mWorld->mNumOutputs, paFloat32 | paNonInterleaved, *outSampleRate, *outNumSamples,
        SC_PortAudioStreamCallback, this);
