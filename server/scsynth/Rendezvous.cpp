...
void AvahiSession::PublishPort(SCRendezvousProtocol proto, short port)
{
    ...
    avahi_threaded_poll_get(mPoll)->timeout_new(
        avahi_threaded_poll_get(mPoll), avahi_elapse_time(&tv, 0, 0), modify_cb, this);
...
void AvahiSession::CreateServices(AvahiClient *client)
{
    ...
    while (entry) {
        ...
        err = avahi_entry_group_add_service(
            mGroup, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, (AvahiPublishFlags)0, mServiceName, type, NULL, NULL,
            entry->mPort, NULL);
...
void HowlSession::PublishPort(sw_discovery session, SCRendezvousProtocol protocol, short portNum)
{
    ...
    sw_result res = sw_discovery_publish(
        session,
        0, // interface
