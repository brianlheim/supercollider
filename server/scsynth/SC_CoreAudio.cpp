...
bool SC_CoreAudioDriver::DriverSetup(int *outNumSamplesPerCallback, double *outSampleRate)
{
    ...
    if (mPreferredSampleRate) {
        ...
        if (UseSeparateIO()) {
            ...
            auto rateListIter = std::set_union(availableSampleRates.begin(),
                                               availableSampleRates.end(),
                                               inputSampleRates.begin(),
                             ...
                             const AudioTimeStamp *inOutputTime, void *defptr)
{
    ...
    for (i = 0; i < inInputData->mNumberBuffers; i++) {
        memcpy(def->mInputBufList->mBuffers[i].mData,
...
#    endif
    ...
    */
    ...
    ret = AudioUnitSetProperty(inputUnit,
