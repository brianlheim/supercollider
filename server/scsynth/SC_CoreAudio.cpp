...
bool SC_CoreAudioDriver::DriverSetup(int *outNumSamplesPerCallback, double *outSampleRate)
{
    ...
    getDefault:
        ...
        if (mOutputDevice == kAudioDeviceUnknown) {
            ...
            err = AudioObjectGetPropertyData(
                kAudioObjectSystemObject, &propertyAddress, 0, NULL, &count, (void *)&mOutputDevice);
        ...
        if (mInputDevice == kAudioDeviceUnknown) {
            ...
            err = AudioObjectGetPropertyData(
                kAudioObjectSystemObject, &propertyAddress, 0, NULL, &count, (void *)&mInputDevice);
    ...
    if (mPreferredHardwareBufferFrameSize) {
        ...
        err = AudioObjectSetPropertyData(
            mOutputDevice, &propertyAddress, 0, NULL, count, &mPreferredHardwareBufferFrameSize);
        ...
        if (UseSeparateIO()) {
            ...
            err = AudioObjectSetPropertyData(
                mInputDevice, &propertyAddress, 0, NULL, count, &mPreferredHardwareBufferFrameSize);
    ...
    if (mPreferredSampleRate) {
        ...
        if (UseSeparateIO()) {
            ...
            auto rateListIter = std::set_union(
                availableSampleRates.begin(), availableSampleRates.end(), inputSampleRates.begin(),
...
#define SR_MIN_VALUE -9999999
            ...
            if (sampleRateSupported) {
                scprintf(
                    "Requested sample rate %f was not available - attempting to use sample rate of %f\n",
...
OSStatus appIOProcSeparateIn(
    AudioDeviceID device, const AudioTimeStamp *inNow, const AudioBufferList *inInputData,
    const AudioTimeStamp *inInputTime, AudioBufferList *outOutputData, const AudioTimeStamp *inOutputTime, void *defptr)
{
    SC_CoreAudioDriver *def = (SC_CoreAudioDriver *)defptr;
    ...
    for (i = 0; i < inInputData->mNumberBuffers; i++) {
        memcpy(
            def->mInputBufList->mBuffers[i].mData, inInputData->mBuffers[i].mData,
...
bool SC_iCoreAudioDriver::DriverSetup(int *outNumSamplesPerCallback, double *outSampleRate)
{
    ...
    *outSampleRate = 44100;
    ...
    ret = AudioUnitSetProperty(
        inputUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input, 1, &enableIO, sizeof(enableIO));
    if (ret != noErr) {
    ...
    enableIO = 0;
    ret = AudioUnitSetProperty(
        inputUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, 0, &enableIO, sizeof(enableIO));
    if (ret != noErr) {
    ...
    */
    ...
    ret = AudioUnitSetProperty(
        inputUnit, kAudioOutputUnitProperty_SetInputCallback, kAudioUnitScope_Input, 0, &inputStruct,
        sizeof(inputStruct));
    ...
    ret = AudioUnitSetProperty(
        unit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, 0, &renderStruct, sizeof(renderStruct));
    ...
    ret = AudioUnitSetProperty(
        inputUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 1, &audioFormat, sizeof(audioFormat));
