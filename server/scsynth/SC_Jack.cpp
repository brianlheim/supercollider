...
SC_JackDriver::SC_JackDriver(struct World *inWorld):
    SC_AudioDriver(inWorld), mClient(0), mInputList(0), mOutputList(0), mMaxOutputLatency(0.)
{}
...
bool SC_JackDriver::DriverSetup(int *outNumSamples, double *outSampleRate)
{
    ...
    mClient = jack_client_open(
        clientName ? clientName : kJackDefaultClientName, serverName ? JackServerName : JackNullOption, NULL,
        serverName);
    if (serverName)
        free(serverName);
    if (clientName)
