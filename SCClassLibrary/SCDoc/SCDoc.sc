...
| this file so you force the docs to     |
\ regenerate.                            /
 ----------------------------------------
    \                                  ___-------___
     \                             _-~~             ~~-_
...
SCDocEntry {
	...
	*newUndocClass {|name|
		...
		doc.undoccmethods = doc.klass.class.methods.collect(_.name).reject { |s| s.asString.last == $_ };
		doc.undocimethods = doc.klass.methods.collect(_.name).reject { |s| s.asString.last == $_ };
	...
	init {|node,aPath|
		...
		// these should be arrays so we don't lose ordering in the autogenerated template
		undoccmethods = Array();
		undocimethods = Array();
	...
	indexUndocumentedMethods {
		...
		var filterFunc = { |methods, filterSet|
			if(methods.notNil) {
				methods.reject { |method|
					filterSet.includes(method.name).not and: { method.name.asString.endsWith($_).not }
				}
			} {
				[]
			}
		...
		undocimethods = undocimethods.addAll(filterFunc.(klass.methods, docmets));

		docmets = doccmethods | privcmethods | ignoreMethods;
		undoccmethods = undoccmethods.addAll(filterFunc.(klass.class.methods, docmets));
	}
