...
void initPyrThread(
    VMGlobals *g, PyrThread *thread, PyrSlot *func, int stacksize, PyrInt32Array *rgenArray, double beats,
    double seconds, PyrSlot *clock, bool runGC);
void initPyrThread(
    VMGlobals *g, PyrThread *thread, PyrSlot *func, int stacksize, PyrInt32Array *rgenArray, double beats,
    double seconds, PyrSlot *clock, bool runGC)
{
...
int prThreadInit(struct VMGlobals *g, int numArgsPushed)
{
    ...
    initPyrThread(
        g, thread, b, stacksize, (PyrInt32Array *)(slotRawObject(&g->thread->randData)), beats, seconds,
        &g->thread->clock, true);
...
int definePrimitiveWithKeys(
    int base, int index, const char *name, PrimitiveHandler handler, PrimitiveWithKeysHandler keyhandler, int numArgs,
    int varArgs)
{
    int tableIndex;
    PyrSymbol *sym;
    ...
    if (def->keyArgs && numKeyArgsPushed) {
        ...
        if (numKeyArgsPushed && methraw->posargs) {
            ...
            for (i = 0; i < numKeyArgsPushed; ++i, key += 2) {
                ...
                if (gKeywordError) {
                    post(
                        "WARNING: keyword arg '%s' not found in call to %s:%s\n", slotRawSymbol(key)->name,
                        slotRawSymbol(&slotRawClass(&meth->ownerclass)->name)->name, slotRawSymbol(&meth->name)->name);
                }
            found:;
            }
...
void initPrimitives()
{
    ...
    definePrimitive(
        base, index++, "_LanguageConfig_getPostInlineWarnings", prLanguageConfig_getPostInlineWarnings, 1, 0);
    definePrimitive(
        base, index++, "_LanguageConfig_setPostInlineWarnings", prLanguageConfig_setPostInlineWarnings, 2, 0);
