...
struct SC_AlsaMidiClient
{
    ...
    int startInputThread()
    {
        ...
        try {
            std::thread inputThread([this] { inputThreadFunc(); });
    ...
    int connectInput(
        int inputIndex, int uid, int (*action)(snd_seq_t *, snd_seq_port_subscribe_t *), const char *actionName);
    int connectOutput(
        int outputIndex, int uid, int (*action)(snd_seq_t *, snd_seq_port_subscribe_t *), const char *actionName);
...
static inline int SC_AlsaMakeUID(int clientID, int portID) { return (clientID << 16) | (portID & 0xFFFF); }
...
void SC_AlsaMidiClient::processEvent(snd_seq_event_t *evt)
{
    ...
    if (compiledOK) {
        ...
        // source
        ++g->sp;
        SetInt(g->sp, SC_AlsaMakeUID(evt->source.client, evt->source.port));

        switch (evt->type) {
        // midi events
        case SND_SEQ_EVENT_NOTEOFF: // noteOff
            ++g->sp;
            SetInt(g->sp, evt->data.note.channel);
            ++g->sp;
            SetInt(g->sp, evt->data.note.note);
            ++g->sp;
            SetInt(g->sp, evt->data.note.velocity);
        ...
        case SND_SEQ_EVENT_PITCHBEND: // bend
            ...
            SetInt(g->sp, evt->data.control.channel);
            ++g->sp;
            SetInt(g->sp, evt->data.control.value + 8192);
...
int SC_AlsaMidiClient::connectInput(
    int inputIndex, int uid, int (*action)(snd_seq_t *, snd_seq_port_subscribe_t *), const char *actionName)
{
    ...
    if ((inputIndex < 0) || (inputIndex >= mNumInPorts))
...
int SC_AlsaMidiClient::connectOutput(
    int outputIndex, int uid, int (*action)(snd_seq_t *, snd_seq_port_subscribe_t *), const char *actionName)
{
    snd_seq_t *seq = mHandle;
    snd_seq_client_info_t *cinfo;
    ...
    if ((outputIndex < 0) || (outputIndex >= mNumOutPorts))
        return errIndexOutOfRange;
...
int initMIDI(int numIn, int numOut)
{
    ...
    for (i = 0; i < numIn; i++) {
        ...
        port = snd_seq_create_simple_port(
            client->mHandle, str, SND_SEQ_PORT_CAP_WRITE | SND_SEQ_PORT_CAP_SUBS_WRITE, SND_SEQ_PORT_TYPE_APPLICATION);
    ...
    for (i = 0; i < numOut; i++) {
        ...
        port = snd_seq_create_simple_port(
            client->mHandle, str, SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_SUBS_READ, SND_SEQ_PORT_TYPE_APPLICATION);
...
int restartMIDI() { return errNone; }
...
int prDisposeMIDIClient(VMGlobals *g, int numArgsPushed) { return disposeMIDI(); }

int prRestartMIDI(VMGlobals *g, int numArgsPushed);
int prRestartMIDI(VMGlobals *g, int numArgsPushed) { return restartMIDI(); }

int prListMIDIEndpoints(struct VMGlobals *g, int numArgsPushed);
int prListMIDIEndpoints(struct VMGlobals *g, int numArgsPushed) { return listMIDIEndpoints(g, g->sp); }
...
void deinitMIDIPrimitives() { cleanUpMIDI(); }
// EOF
