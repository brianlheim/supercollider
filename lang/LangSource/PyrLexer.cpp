...
symbol3 : {
    ...
    for (; yylen < MAXYYLEN;) {
        ...
        if (c == '\n' || c == '\r') {
            post(
                "Symbol open at end of line on line %d of %s\n", startline + errLineOffset,
...
ClassDependancy *newClassDependancy(
    PyrSymbol *className, PyrSymbol *superClassName, PyrSymbol *fileSym, int startPos, int endPos, int lineOffset)
{
        ...
        if (sym && (sym->flags & sym_Class)) {
            if (sym->classdep) {
                if (sym->classdep->superClassName->classdep) {
                    ...
                    error(
                        "Superclass '%s' of class '%s' is not defined in any file.\n%s\n",
...
SCLANG_DLLEXPORT_C void runLibrary(PyrSymbol *selector)
{
    ...
    try {
        ...
        if (meth) {
            ...
            post(
                "caught exception in runLibrary %s:%s %3d\n",
