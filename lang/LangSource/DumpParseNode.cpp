    ...
    case tagObj:
        if (slotRawObject(slot)) {
            ...
            if (classptr == class_class) {
                sprintf(
                    str, "class %s (%p)", slotRawSymbol(&((PyrClass *)slotRawObject(slot))->name)->name,
                    slotRawObject(slot));
                ...
                if (NotNil(nextcontext)) {
                    ...
                    if (isKindOf(slotRawObject(context), class_method)) {
                        sprintf(
                            str, "instance of FunctionDef in Method %s:%s",
                ...
                if (!slotRawFrame(slot)) {
                    ...
                    sprintf(
                        str, "Frame (%p) of %s:%s", slotRawObject(slot),
                ...
                sprintf(
                    str, "instance of %s (%p, size=%d, set=%d)", slotRawSymbol(&classptr->name)->name,
...
static void printObject(PyrSlot *slot, PyrObject *obj, char *str)
{
    ...
    if (classptr == class_class) {
        ...
        sprintf(
            str, "%s:%s", slotRawSymbol(&slotRawClass(&slotRawMethod(slot)->ownerclass)->name)->name,
