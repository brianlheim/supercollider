...
void PyrClassExtNode::compile(PyrSlot *result)
{
    ...
    if (!classobj) {
        ...
        error(
            "Class extension for nonexistent class '%s'\n     In file:'%s'\n", slotRawSymbol(&mClassName->mSlot)->name,
...
PyrClassNode *newPyrClassNode(
    PyrSlotNode *className, PyrSlotNode *superClassName, PyrVarListNode *varlists, PyrMethodNode *methods,
    PyrSlotNode *indexType)
{
...
void fillClassPrototypes(PyrClassNode *node, PyrClass *classobj, PyrClass *superclassobj)
{
    ...
    if (superclassobj && NotNil(&superclassobj->iprototype) && slotRawObject(&superclassobj->iprototype)->size) {
        memcpy(
            slotRawObject(&classobj->iprototype)->slots, slotRawObject(&superclassobj->iprototype)->slots,
        ...
        memcpy(
            slotRawSymbolArray(&classobj->instVarNames)->symbols,
...
void PyrClassNode::compile(PyrSlot *result)
{
    ...
    if (classobj) {
        ...
        if (varsDiffer || superclassesDiffer || indexTypesDiffer) {
            if (isIntrinsic) {
                if (superclassesDiffer) {
                    error(
                        "Superclass of '%s' does not match intrinsic definition.\n",
                        slotRawSymbol(&mClassName->mSlot)->name);
                ...
                if (indexTypesDiffer) {
                    error(
                        "Index type of '%s' does not match intrinsic definition.\n",
                        slotRawSymbol(&mClassName->mSlot)->name);
                    nodePostErrorLine((indexType ? (PyrParseNode *)mIndexType : (PyrParseNode *)mClassName));
                    compileErrors++;
                }
                error(
                    "Class '%s' declaration doesn't match intrinsic definition.\n",
                    slotRawSymbol(&mClassName->mSlot)->name);
                return;
            } else {
                shouldRecompileSubclasses = true;
        ...
        memcpy(
            slotRawObject(&metaclassobj->iprototype)->slots, slotRawObject(&class_class->iprototype)->slots,
            sizeof(PyrSlot) * classClassNumInstVars);
        memcpy(
            slotRawSymbolArray(&metaclassobj->instVarNames)->symbols,
        ...
        reallocClassObj(
            classobj, mVarTally[varInst] + mNumSuperInstVars, mVarTally[varClass], mVarTally[varConst], numInstMethods,
            indexType, 0);
        ...
        classobj = newClassObj(
            metaclassobj, slotRawSymbol(&mClassName->mSlot), superClassName, mVarTally[varInst] + mNumSuperInstVars,
...
PyrMethodNode *newPyrMethodNode(
    PyrSlotNode *methodName, PyrSlotNode *primitiveName, PyrArgListNode *arglist, PyrVarListNode *varlist,
    PyrParseNode *body, int isClassMethod)
{
...
int compareCallArgs(PyrMethodNode *node, PyrCallNode *cnode, int *varIndex, PyrClass **specialClass)
{
    ...
    if (slotRawSymbol(&nameNode->mSlot) == s_this) {
        ...
        varFound = findVarName(
            gCompilingBlock, &classobj, slotRawSymbol(&nameNode->mSlot), &varType, &varLevel, varIndex, NULL);
        if (!varFound)
            return methNormal;
            ...
            for (i = 1; i < numArgs; ++i, vardef = (PyrVarDefNode *)vardef->mNext) {
                ...
                for (j = 0; j < i; ++j) {
                    if (methargs[j] == slotRawSymbol(varslot)) {
                        error(
                            "Argument '%s' already declared in %s:%s\n", slotRawSymbol(varslot)->name,
                            slotRawSymbol(&gCompilingClass->name)->name, slotRawSymbol(&gCompilingMethod->name)->name);
    ...
    if (mVarlist) {
        ...
        for (i = 0; i < numVars; ++i, vardef = (PyrVarDefNode *)vardef->mNext) {
            ...
            for (j = 0; j < numArgNames; ++j) {
                if (methargs[j] == slotRawSymbol(varslot)) {
                    error(
                        "Variable '%s' already declared in %s:%s\n", slotRawSymbol(varslot)->name,
                        slotRawSymbol(&gCompilingClass->name)->name, slotRawSymbol(&gCompilingMethod->name)->name);
                    nodePostErrorLine((PyrParseNode *)vardef);
                    compileErrors++;
                }
...
PyrCallNode *
newPyrCallNode(PyrSlotNode *selector, PyrParseNode *arglist, PyrParseNode *keyarglist, PyrParseNode *blocklist)
{
    ...
    if (numKeyArgs > 0 || (numArgs > 15 && !(selType == selSwitch || selType == selCase))) {
        ...
        case selBinary:
            ...
            if (index == opAdd && argnode2->mClassno == pn_PushLitNode && IsInt(&((PyrPushLitNode *)argnode2)->mSlot) &&
                ...
                index == opSub && argnode2->mClassno == pn_PushLitNode && IsInt(&((PyrPushLitNode *)argnode2)->mSlot) &&
                slotRawInt(&((PyrPushLitNode *)argnode2)->mSlot) == 1) {
                COMPILENODE(argnode, &dummy, false);
    ...
    if (isSuper) {
        ...
        case selBinary:
            if (arg3) {
                ...
                index == opAdd && arg2->mClassno == pn_PushLitNode && IsInt(&((PyrPushLitNode *)arg2)->mSlot) &&
                slotRawInt(&((PyrPushLitNode *)arg2)->mSlot) == 1) {
                COMPILENODE(arg1, &dummy, false);
                compileOpcode(opPushSpecialValue, opsvPlusOne);
            } else if (
                index == opSub && arg2->mClassno == pn_PushLitNode && IsInt(&((PyrPushLitNode *)arg2)->mSlot) &&
                slotRawInt(&((PyrPushLitNode *)arg2)->mSlot) == 1) {
                COMPILENODE(arg1, &dummy, false);
                compileTail();
                compileOpcode(opPushSpecialValue, opsvMinusOne);
        ...
        for (i = 0; i < numArgs; ++i, vardef = (PyrVarDefNode *)vardef->mNext) {
            ...
            for (j = 0; j < i; ++j) {
                if (blockargs[j] == slotRawSymbol(varslot)) {
                    error(
                        "Function argument '%s' already declared in %s:%s\n", slotRawSymbol(varslot)->name,
                        slotRawSymbol(&gCompilingClass->name)->name, slotRawSymbol(&gCompilingMethod->name)->name);
                    nodePostErrorLine((PyrParseNode *)vardef);
                    compileErrors++;
                }
    ...
    if (numVars) {
        ...
        for (i = 0; i < numVars; ++i, vardef = (PyrVarDefNode *)vardef->mNext) {
            ...
            for (j = 0; j < numArgNames; ++j) {
                if (blockargs[j] == slotRawSymbol(varslot)) {
                    error(
                        "Function variable '%s' already declared in %s:%s\n", slotRawSymbol(varslot)->name,
                        slotRawSymbol(&gCompilingClass->name)->name, slotRawSymbol(&gCompilingMethod->name)->name);
                    nodePostErrorLine((PyrParseNode *)vardef);
                    compileErrors++;
                }
