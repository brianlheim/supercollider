...
SC_TerminalClient::SC_TerminalClient(const char *name):
    SC_LanguageClient(name), mReturnCode(0), mUseReadline(false), mWork(mIoService), mTimer(mIoService),
#ifndef _WIN32
...
void SC_TerminalClient::startInputRead()
{
    ...
    if (mUseReadline)
        ...
        mStdIn.async_read_some(
            boost::asio::buffer(inputBuffer), boost::bind(&SC_TerminalClient::onInputRead, this, _1, _2));
#else
    mStdIn.async_wait([&](const boost::system::error_code &error) {
        if (error)
            ...
            ::ReadFile(
                GetStdHandle(STD_INPUT_HANDLE), inputBuffer.data(), inputBuffer.size(), &bytes_transferred, nullptr);
