...
public:
    Document(
        bool isPlainText, const QByteArray &id = QByteArray(), const QString &title = QString(),
...
public slots:
    ...
    Document *open(
        const QString &path, int initialCursorPosition = -1, int selectionLength = 0, bool addToRecent = true,
        const QByteArray &id = QByteArray(), bool syncLang = true);
    void clearRecents();
...
private:
    Document *createDocument(
        bool isPlainText = false, const QByteArray &id = QByteArray(), const QString &title = QString(),
        const QString &text = QString());
