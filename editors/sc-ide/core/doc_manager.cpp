...
Document::Document(bool isPlainText, const QByteArray &id, const QString &title, const QString &text):
    mId(id), mDoc(new QTextDocument(text, this)), mTitle(title), mIndentWidth(4), mHighlighter(0),
    mKeyDownActionEnabled(false), mKeyUpActionEnabled(false), mMouseDownActionEnabled(false),
    mMouseUpActionEnabled(false), mTextChangedActionEnabled(false), mLastActiveEditor(0), mInitialSelectionStart(0),
    mInitialSelectionRange(0), mEditable(true), mPromptsToSave(true)
{
    mTmpCoalCount = 0;
    ...
    connect(
        Main::instance(), SIGNAL(applySettingsRequest(Settings::Manager *)), this,
...
DocumentManager::DocumentManager(Main *main, Settings::Manager *settings):
    QObject(main), mTextMirrorEnabled(true), mCurrentDocument(NULL), mGlobalKeyDownEnabled(false),
...
Document *
DocumentManager::createDocument(bool isPlainText, const QByteArray &id, const QString &title, const QString &text)
{
...
Document *DocumentManager::open(
    const QString &path, int initialCursorPosition, int selectionLength, bool toRecent, const QByteArray &id,
    bool syncLang)
{
    QFileInfo info(path);
    ...
    if (info.suffix() == QStringLiteral("rtf")) {
        ...
        QMessageBox::warning(
            NULL, QString(tr("Opening RTF File")),
            QString(tr("Warning: RTF file will be converted to plain-text scd file.")));
    }

    closeSingleUntitledIfUnmodified();

    const bool fileIsPlainText =
        !(info.suffix() == QStringLiteral("sc") || (info.suffix() == QStringLiteral("scd")) ||
        ...
        if (doc) {
            ...
            if (document) {
                ...
                if (document == mCurrentDocument) {
                    disconnect(
                        document->textDocument(), SIGNAL(contentsChange(int, int, int)), this,
...
void DocumentManager::setActiveDocument(Document *document)
{
    if (mCurrentDocument)
        disconnect(
            mCurrentDocument->textDocument(), SIGNAL(contentsChange(int, int, int)), this,
            SLOT(updateCurrentDocContents(int, int, int)));
    if (document) {
...
void DocumentManager::updateCurrentDocContents(int position, int charsRemoved, int charsAdded)
{
    ...
    if (mCurrentDocument->textChangedActionEnabled()) {
        ...
        Main::evaluateCode(
            QStringLiteral("Document.findByQUuid(\'%1\').textChanged(%2, %3, %4);")
