    ...
    ControlElement element, const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    case QStyle::CE_TabBarTab: {
        ...
        if (!selected && !(tabOption->selectedPosition == QStyleOptionTab::NextIsSelected) &&
            !(tabOption->position == QStyleOptionTab::End) && !(tabOption->position == QStyleOptionTab::OnlyOneTab)) {
...
QSize Style::sizeFromContents(
    ContentsType type, const QStyleOption *option, const QSize &contentsSize, const QWidget *widget) const
{
    if (shouldNotHandle(widget)) {
