...
ScCodeEditor::ScCodeEditor(Document *doc, QWidget *parent):
    GenericCodeEditor(doc, parent), mSpaceIndent(true), mBlinkDuration(600), mMouseBracketMatch(false),
...
void ScCodeEditor::gotoNextRegion()
{
    ...
    while (it.isValid()) {
        ...
        if (bracketPair.first.isValid() && bracketPair.second.isValid() &&
            bracketPair.first.position() > cursorPosition && bracketPairDefinesRegion(bracketPair)) {
