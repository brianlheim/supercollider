...
static bool findInBlock(
    QTextDocument *doc, const QTextBlock &block, const QRegExp &expr, int offset, QTextDocument::FindFlags options,
    QTextCursor &cursor)
{
...
void GenericCodeEditor::doKeyAction(QKeyEvent *ke)
{
    ...
    Main::evaluateCodeIfCompiled(
        QStringLiteral("Document.findByQUuid(\'%1\').%2(%3, %4, %5, %6)")
...
void GenericCodeEditor::mousePressEvent(QMouseEvent *e)
{
    if (mDoc->mouseDownActionEnabled()) {
        ...
        Main::evaluateCodeIfCompiled(
            QStringLiteral("Document.findByQUuid(\'%1\').mouseDown(%2, %3, %4, %5, 1)")
                .arg(mDoc->id().constData())
                .arg(e->x())
                .arg(e->y())
                .arg(e->modifiers())
...
void GenericCodeEditor::mouseDoubleClickEvent(QMouseEvent *e)
{
    if (mDoc->mouseDownActionEnabled()) {
        ...
        Main::evaluateCodeIfCompiled(
            QStringLiteral("Document.findByQUuid(\'%1\').mouseDown(%2, %3, %4, %5, 2)")
                .arg(mDoc->id().constData())
                .arg(e->x())
                .arg(e->y())
                .arg(e->modifiers())
...
void GenericCodeEditor::mouseReleaseEvent(QMouseEvent *e)
{
    if (mDoc->mouseUpActionEnabled()) {
        ...
        Main::evaluateCodeIfCompiled(
            QStringLiteral("Document.findByQUuid(\'%1\').mouseUp(%2, %3, %4, %5)")
                .arg(mDoc->id().constData())
                .arg(e->x())
                .arg(e->y())
                .arg(e->modifiers())
