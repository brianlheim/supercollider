...
CompletionMenu *
AutoCompleter::menuForClassMethodCompletion(CompletionDescription const &completion, ScCodeEditor *editor)
{
    using namespace ScLanguage;
...
void AutoCompleter::triggerMethodCallAid(bool explicitTrigger)
{
    ...
    qDebug(
        "Method call: found call: %s:%s", receiverClass ? qPrintable(receiverClass->name.get()) : "",
...
bool AutoCompleter::testMethodCall(
    const MethodCall &call, int cursorPos, int &outArgNum, TokenIterator &outArgNameToken, bool strict)
{
