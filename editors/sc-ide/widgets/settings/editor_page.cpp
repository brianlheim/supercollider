...
EditorPage::EditorPage(QWidget *parent): QWidget(parent), fontDatabase(new QFontDatabase), ui(new Ui::EditorConfigPage)
{
    ui->setupUi(this);
    ...
    connect(
        ui->textFormats, SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)), this,
...
void EditorPage::loadThemeFormats(Theme &theme)
{
    ...
    static QStringList strings =
        QStringList() << tr("Current Line") << tr("Search Result") << tr("Matching Brackets")
                      << tr("Mismatched Brackets") << tr("Evaluated Code") << tr("Line Numbers") << tr("Selected Text")
                      << tr("Post Window Text") << tr("Whitespace") << tr("Keyword") << tr("Built-in Value")
                      << tr("Environment Variable") << tr("Class") << tr("Number") << tr("Symbol") << tr("String")
                      << tr("Char") << tr("Comment") << tr("Primitive") << tr("Post Window Error")
                      << tr("Post Window Warning") << tr("Post Window Success") << tr("Post Window Emphasis");
...
QTreeWidgetItem *EditorPage::addTextFormat(
    const QString &name, const QString &key, const QTextCharFormat &format, const QTextCharFormat &defaultFormat)
{
...
void EditorPage::dialogCopyTheme()
{
    ...
    QString newThemeName = QInputDialog::getText(
        this, tr("Copy theme"), tr("copy the selected theme:"), QLineEdit::Normal, themeName, &ok);
    ...
    if (ok && !newThemeName.isEmpty()) {
        ...
        if (themes.indexOf(newThemeName) >= 0) {
            QMessageBox::information(
                this, tr("Theme Already Existing"), tr("This theme already exists, pick another one\n"));
        } else {
