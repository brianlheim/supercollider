    ...
    mDocDialog(0)
{
    ...
    mProjectTreeDocklet = new ProjectTreeDocklet(this);
    mProjectTreeDocklet->setObjectName("project-tree-dock");
    addDockWidget(Qt::LeftDockWidgetArea, mProjectTreeDocklet->dockWidget());
    ...
    //project
    connect(mProjectTreeDocklet->tree(), SIGNAL(clicked(QString)), mMain->documentManager(), SLOT(open(QString)));
...
void MainWindow::createActions()
{
    ...
    mActions[ToggleProjectDockelt] = action = mProjectTreeDocklet->toggleViewAction();
    action->setIcon( QIcon::fromTheme("folder") );
    action->setStatusTip(tr("Show/hide Project tree docklet"));
    settings->addAction( mProjectTreeDocklet->toggleViewAction(),
                         "ide-docklet-project", ideCategory );
...
void MainWindow::createMenus()
{
    ...
    submenu->addAction( mProjectTreeDocklet->toggleViewAction() );
...
void MainWindow::updateProjectObjects()
{
    ...
    if (projectOpen)
        mProjectTreeDocklet->setProjectFile(configFile);

    mProjectTreeDocklet->setVisible(projectOpen);
    mActions[ProjectClose]->setEnabled(projectOpen);
    mActions[ProjectSaveAs]->setEnabled(projectOpen);
    mActions[ToggleProjectDockelt]->setEnabled(projectOpen);
}

//NOTE: check that yaml config file is still really a project. It could have
// been edited manually meanwhile. Check both the config file in the settings
// and in the session.
