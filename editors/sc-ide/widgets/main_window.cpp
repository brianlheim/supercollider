...
MainWindow::MainWindow(Main *main): mMain(main), mClockLabel(0), mDocDialog(0)
{
    ...
    connect(
        main->scProcess(), SIGNAL(stateChanged(QProcess::ProcessState)), this,
    ...
    connect(
        mEditors, SIGNAL(currentDocumentChanged(Document *)), mDocumentsDocklet->list(), SLOT(setCurrent(Document *)),
        Qt::QueuedConnection);
    connect(
        mDocumentsDocklet->list(), SIGNAL(updateTabsOrder(QList<Document *>)), mEditors,
        SLOT(updateTabsOrder(QList<Document *>)));
    connect(
        mEditors, SIGNAL(updateDockletOrder(int, int)), mDocumentsDocklet->list(), SLOT(updateDockletOrder(int, int)),
        Qt::QueuedConnection);
...
bool MainWindow::close(Document *doc)
{
    if (doc->textDocument()->isModified() && doc->promptsToSave()) {
        ...
        ret = QMessageBox::warning(
            mInstance, tr("SuperCollider IDE"),
...
bool MainWindow::save(Document *doc, bool forceChoose, bool saveInExtensionFolder)
{
    ...
    if ((!forceChoose) && documentHasPath) {
        ...
        if (!fileIsWritable) {
            QMessageBox::warning(
                instance(), tr("Saving read-only file"),
    ...
    if (forceChoose || !documentHasPath || !fileIsWritable) {
        ...
        QStringList filters = QStringList()
                              << tr("All Files (*)") << tr("SuperCollider Document (*.scd)")
                              << tr("SuperCollider Class File (*.sc)") << tr("SuperCollider Help Source (*.schelp)");
...
void MainWindow::lookupDocumentationForCursor()
{
    ...
    if (widget && methodIdx != -1) {
        widget->metaObject()->method(methodIdx).invoke(
            widget, Qt::DirectConnection, Q_RETURN_ARG(bool, documentationOpened));
    };
