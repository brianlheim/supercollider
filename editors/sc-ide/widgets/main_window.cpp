...
MainWindow::MainWindow(Main *main): mMain(main), mClockLabel(0), mDocDialog(0)
{
    ...
    connect(mEditors,
            SIGNAL(currentDocumentChanged(Document *)),
            mDocumentsDocklet->list(),
            SLOT(setCurrent(Document *)),
            Qt::QueuedConnection);
    connect(mDocumentsDocklet->list(),
            SIGNAL(updateTabsOrder(QList<Document *>)),
            mEditors,
            SLOT(updateTabsOrder(QList<Document *>)));
    connect(mEditors,
            SIGNAL(updateDockletOrder(int, int)),
            mDocumentsDocklet->list(),
            SLOT(updateDockletOrder(int, int)),
            Qt::QueuedConnection);
...
bool MainWindow::close(Document *doc)
{
    if (doc->textDocument()->isModified() && doc->promptsToSave()) {
        ...
        ret = QMessageBox::warning(mInstance,
...
bool MainWindow::save(Document *doc, bool forceChoose, bool saveInExtensionFolder)
{
    ...
    if ((!forceChoose) && documentHasPath) {
        ...
        if (!fileIsWritable) {
            QMessageBox::warning(instance(),
                                 tr("Saving read-only file"),
