...
bool QObjectProxy::invokeMethod(const char *method, PyrSlot *retSlot, PyrSlot *argSlot, Qt::ConnectionType ctype)
{
    ...
    // do it!
    bool success = mm.invoke(
        qObject, ctype, rarg, args[0].toGenericArgument(), args[1].toGenericArgument(), args[2].toGenericArgument(),
        args[3].toGenericArgument(), args[4].toGenericArgument(), args[5].toGenericArgument(),
        args[6].toGenericArgument(), args[7].toGenericArgument(), args[8].toGenericArgument(),
...
bool QObjectProxy::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (!preProcessEvent(watched, event, *d, args)) {
        qcProxyDebugMsg(
            3, QStringLiteral("Event (%1, %2) not handled, forwarding to the widget")
                   .arg(type)
    ...
    qcProxyDebugMsg(
        1, QStringLiteral("Will handle event (%1, %2) -> (%3, %4)")
               .arg(type)
               .arg(event->spontaneous() ? "spontaneous" : "inspontaneous")
               .arg(d->method->name)
