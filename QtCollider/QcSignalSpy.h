...
public:
    QcMethodSignalHandler(
        QObjectProxy *proxy, const char *sigName, PyrSymbol *handler,
...
protected:
    ...
    {
        qcDebugMsg(
            1, QString("SIGNAL: '%1' handled by method '%2'")
                   .arg(
                       _proxy->object() ? QString(_proxy->object()->metaObject()->method(_sigId).methodSignature())
                                        : "unknown")
                   .arg(_handler->name));
...
public:
    QcFunctionSignalHandler(
        QObjectProxy *proxy, const char *sigName, PyrObject *handler,
        Qt::ConnectionType conType = Qt::QueuedConnection):
        QcSignalSpy(proxy, sigName, conType),
        _handler(handler)
    {}
...
protected:
    ...
    {
        qcDebugMsg(
            1, QString("SIGNAL: '%1' handled by a Function")
                   .arg(
                       _proxy->object() ? QString(_proxy->object()->metaObject()->method(_sigId).methodSignature())
                                        : "unknown"));
