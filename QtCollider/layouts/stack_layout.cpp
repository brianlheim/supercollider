        ...
        if (fw && (prev && prev->isAncestorOf(fw))) { // focus was on old page
            ...
            if (QWidget *nfw = next->focusWidget())
                ...
                while ((i = i->nextInFocusChain()) != fw) {
                    if (((i->focusPolicy() & Qt::TabFocus) == Qt::TabFocus) && !i->focusProxy() &&
                        i->isVisibleTo(next) && i->isEnabled() && next->isAncestorOf(i)) {
                        i->setFocus();
                        break;
                    }
                }
...
static QSize smartMinSize(
    const QSize &sizeHint, const QSize &minSizeHint, const QSize &minSize, const QSize &maxSize,
