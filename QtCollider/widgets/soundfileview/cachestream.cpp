...
SoundCacheStream::SoundCacheStream():
    SoundStream(0, 0.0, 0.0), _caches(0), _fpu(0.0), _dataOffset(0), _dataSize(0), _ready(false), _loading(false),
    _loadProgress(0)
{
...
void SoundCacheStream::load(
    SNDFILE *sf, const SF_INFO &info, sf_count_t beg, sf_count_t dur, int maxFramesPerUnit, int maxRawFrames)
{
...
bool SoundCacheStream::displayData(
    int ch, double f_beg, double f_dur, float *minBuffer, float *maxBuffer, float *minRMS, float *maxRMS,
    int bufferSize)
{
...
void SoundCacheLoader::run()
{
    ...
    while (i < size) {
        ...
        for (ch = 0; ch < channels; ++ch) {
            sfStream.integrate(
                ch, beg, dur, chanCaches[ch].min + i, chanCaches[ch].max + i, chanCaches[ch].sum + i,
