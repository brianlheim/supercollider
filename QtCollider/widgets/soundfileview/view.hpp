...
public:
    ...
    virtual bool displayData(
        int channel, double offset, double duration, float *minBuffer, float *maxBuffer, float *minRMS, float *maxRMS,
        int bufferSize) = 0;
...
protected:
    ...
    SoundStream(int channels, sf_count_t beginning, sf_count_t duration): _ch(channels), _beg(beginning), _dur(duration)
    {}
...
public:
    ...
    bool integrate(
        int channel, double offset, double duration, float *minBuffer, float *maxBuffer, float *sumBuffer,
