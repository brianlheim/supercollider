...
static void qcGetMethods(const QMetaObject *mo, bool getPlain, bool getSignals, bool getSlots, PyrSlot *r, VMGlobals *g)
{
...
QC_LANG_PRIMITIVE(QObject_DisconnectMethod, 2, PyrSlot *r, PyrSlot *a, VMGlobals *g)
{
    ...
    qcSCObjectDebugMsg(
        1, slotRawObject(r), QStringLiteral("DISCONNECT METHOD: %1 -> %2").arg(signal->name).arg(handler->name));
...
QC_LANG_PRIMITIVE(QObject_ConnectSlot, 3, PyrSlot *r, PyrSlot *a, VMGlobals *g)
{
    ...
    qcSCObjectDebugMsg(
        1, slotRawObject(r), QStringLiteral("CONNECT TO SLOT: %1 -> %2").arg(symSig->name).arg(symSlot->name));
    ...
    if (!sndProxy->object() || !rcvProxy->object()) {
        ...
        ok = QObject::connect(
            sndProxy->object(), strSig.toStdString().c_str(), rcvProxy->object(), strSlot.toStdString().c_str());
    }
    sndProxy->unlock();
    rcvProxy->unlock();
...
QC_LANG_PRIMITIVE(QObject_InvokeMethod, 3, PyrSlot *r, PyrSlot *a, VMGlobals *g)
{
    ...
    qcSCObjectDebugMsg(
        1, slotRawObject(r), QStringLiteral("INVOKE: '%1' [%2]").arg(method->name).arg(sync ? "SYNC" : "ASYNC"));
